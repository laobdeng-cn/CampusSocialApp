...
router.post('/posts/:id/favorite', requireAuth, async (request, response, next) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(request.params.id)) {
      console.warn('[posts:favorite] invalid id', {
        postId: request.params.id,
        userId: String(request.user?._id || ''),
      });
      response.status(404).json({ message: '帖子不存在或暂未同步到后端' });
      return;
    }
...
router.get('/posts/:id/comments', async (request, response, next) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(request.params.id)) {
      console.warn('[posts:comments:list] invalid id', { postId: request.params.id });
...
router.post('/posts/:id/comments', requireAuth, async (request, response, next) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(request.params.id)) {
      console.warn('[posts:comments:create] invalid id', {
        postId: request.params.id,
        userId: String(request.user?._id || ''),
      });
      response.status(404).json({ message: '帖子不存在或暂未同步到后端' });
