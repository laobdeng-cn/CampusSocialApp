...
class CampusRepository {
  ...
  List<CampusPost> _stripFrontendDemoPostsV2(Iterable<CampusPost> posts) {
    return posts
        .where((post) => !_isFrontendDemoPostV2(post))
        .toList(growable: false);
  }

  CampusFeed _stripFrontendDemoFeedV2(CampusFeed feed) {
    return CampusFeed(
      users: feed.users,
      posts: _stripFrontendDemoPostsV2(feed.posts),
      activities: feed.activities,
      groups: feed.groups
          .map(
            (group) => group.copyWith(
              discussions: _stripFrontendDemoPostsV2(group.discussions),
            ),
          )
          .toList(growable: false),
      topics: feed.topics
          .map(
            (topic) =>
                topic.copyWith(posts: _stripFrontendDemoPostsV2(topic.posts)),
          )
          .toList(growable: false),
    );
  }
  ...
  CampusFeed get cachedFeed => _withoutDemoPosts(_cachedFeed);

  CampusFeed _withoutDemoPosts(CampusFeed feed) {
    bool isRealPost(CampusPost post) => post.id.trim().isNotEmpty;

    final realPosts = feed.posts.where(isRealPost).toList(growable: false);
    final realGroups = feed.groups
        .map(
          (group) => group.copyWith(
        ...
        .toList(growable: false);
    final realTopics = feed.topics
        .map(
          (topic) => topic.copyWith(
            posts: topic.posts.where(isRealPost).toList(growable: false),
          ),
        )
        .toList(growable: false);
  ...
  void _emitFeedChanged() {
    _cachedFeed = _withoutDemoPosts(_cachedFeed);
  ...
  Future<CampusPost> createPost({
    ...
    _cachedFeed = CampusFeed(
      ...
      posts: [
        post,
        ..._cachedFeed.posts.where(
          (cachedPost) => cachedPost.id != post.id && !_isDemoPost(cachedPost),
        ),
      ],
  ...
  Future<CampusPost> togglePostLike(CampusPost post) async {
    ...
    if (!RegExp(r'^[a-fA-F0-9]{24}$').hasMatch(id)) {
      throw const CampusApiException('演示帖子暂不支持点赞，请发布真实帖子后再试');
    }
  ...
  Future<CampusPost> togglePostFavorite(CampusPost post) async {
    final id = _requirePostId(post);
    if (!RegExp(r'^[a-fA-F0-9]{24}$').hasMatch(id)) {
      throw const CampusApiException('演示帖子暂不支持收藏，请发布真实帖子后再试');
    }
  ...
  Future<({CampusComment comment, CampusPost post})> createComment({
    ...
    final id = _requirePostId(post);
    if (!RegExp(r'^[a-fA-F0-9]{24}$').hasMatch(id)) {
      throw const CampusApiException('演示帖子暂不支持评论，请发布真实帖子后再试');
    }
  ...
  Future<CampusActivity> cancelJoinActivity(CampusActivity activity) async {
    final id = _requireActivityId(activity);
    final nextActivity = await _apiClient.cancelJoinActivity(
      token: _requireToken(),
      activityId: id,
    );
  ...
  Future<CampusUser> followUser(CampusUser user) async {
    final id = _requireUserId(user);
    final next = await _apiClient.followUser(
      token: _requireToken(),
      userId: id,
    );
  ...
  Future<CampusUser> unfollowUser(CampusUser user) async {
    final id = _requireUserId(user);
    final next = await _apiClient.unfollowUser(
      token: _requireToken(),
      userId: id,
    );
    _emitSync(CampusEventType.profileChanged, refId: id, payload: next);
