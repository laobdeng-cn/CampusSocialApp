...
class CreateActivityScreen extends StatefulWidget {
  ...
  final CampusActivity? editingActivity;
...
class _CreateActivityScreenState extends State<CreateActivityScreen> {
  static const _categories = ['社团招新', '讲座', '体育', '文艺', '志愿', '比赛', '校园服务'];
  ...
  void initState() {
    ...
    if (activity == null) {
      ...
      _dateController.text = '6月20日（周六）';
      _timeController.text = '19:00 - 21:00';
      _locationController.text = '大学生活动中心';
      _hostController.text = widget.groupContext?.name ?? '校学生会';
    ...
    _category = _categories.contains(activity.category)
        ? activity.category
        : '讲座';
    _dateController.text = activity.date;
    _timeController.text = activity.time;
    _locationController.text = activity.location;
    _hostController.text = activity.host;
    _capacityController.text = '${activity.capacity}';
    _priceController.text = activity.price;
  ...
  void dispose() {
    ...
    _checkInCodeController.dispose();
    super.dispose();
  }
  ...
  List<String> _readTags() {
    ...
    if (raw.isEmpty) return [_category];
  ...
  String? _validate() {
    if (_titleController.text.trim().isEmpty) return '请输入活动标题';
    if (_dateController.text.trim().isEmpty) return '请输入活动日期';
    if (_timeController.text.trim().isEmpty) return '请输入活动时间';
    if (_locationController.text.trim().isEmpty) return '请输入活动地点';
    if (_hostController.text.trim().isEmpty) return '请输入主办方';
    if (_descriptionController.text.trim().isEmpty) return '请输入活动介绍';
    if (_checkInCodeController.text.trim().isEmpty) return '请输入签到口令';

    if (_checkInCodeController.text.trim().length < 4) return '签到口令至少 4 位';

    final capacity = int.tryParse(_capacityController.text.trim());
    if (capacity == null || capacity <= 0) return '请输入正确的名额人数';

    return null;
  }

  Future<String> _posterForSubmit() async {
    final selected = _selectedPosterPath;
    if (selected != null && selected.isNotEmpty) {
      return CampusRepository.instance.uploadImage(
        selected,
        purpose: 'activity',
      );
    }
  ...
  Future<void> _submit() async {
    ...
    if (error != null) {
      _showFeatureMessage(context, error);
      return;
    }
            ...
            child: Column(
              children: [
                ...
                const Align(
                  ...
                  child: Text(
                    '学生在活动签到页输入该口令完成签到，发布后也可在“我发起的”中重置。',
                    style: TextStyle(
                      color: AppColors.muted,
                      fontSize: 12,
                      height: 1.35,
            ...
            child: Column(
              children: [
                SwitchListTile(
                  ...
                  subtitle: const Text('开启后，活动详情后续可展示评论互动'),
                ),
                const Divider(height: 1),
                SwitchListTile(
                  value: _publicDisplay,
                  onChanged: _isSubmitting
                      ? null
                      : (value) => setState(() => _publicDisplay = value),
                  title: const Text(
                    '公开展示',
                    style: TextStyle(fontWeight: FontWeight.w900),
                  ),
                  subtitle: const Text('公开后会出现在活动首页和全部活动列表'),
                ),
              ],
            ),
          ),
...
class _MyCreatedActivitiesScreenState extends State<MyCreatedActivitiesScreen> {
  ...
  void initState() {
    super.initState();
    _activitiesFuture = CampusRepository.instance.fetchCreatedActivities();
  }
  ...
  Future<void> _openEdit(CampusActivity activity) async {
    final changed = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => CreateActivityScreen(editingActivity: activity),
      ),
    );
  ...
  Future<void> _deleteActivity(CampusActivity activity) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text('确定删除「${activity.title}」吗？删除后活动列表中将不再展示。'),
          actions: [
            TextButton(
    ...
    try {
      ...
      // 页面层兜底：
      // DELETE 成功后后端可能返回空 body，接口层解析失败会进入 catch。
      // 重新拉取我发起的列表，如果目标活动已经不存在，就按删除成功处理。
      try {
        final latest = await CampusRepository.instance.fetchCreatedActivities();
        final stillExists = latest.any((item) => item.id == activity.id);

        if (!mounted) return;
        if (!stillExists) {
          _showFeatureMessage(context, '活动已删除');
          setState(() {
            _activitiesFuture = Future.value(latest);
          });
          return;
        }
      } catch (_) {}
  ...
  Future<void> _resetCode(CampusActivity activity) async {
    ...
    try {
      ...
      debugPrint('resetActivityCheckInCode request failed: $error');

      if (!mounted) return;
      _showFeatureMessage(
        context,
        '重置口令失败，请看后端终端日志：/api/activities/${activity.id}/checkin-code/reset',
      );
      return;
    }
    ...
    try {
      ...
      debugPrint('refresh created activities after reset failed: $error');
    }
  ...
  Future<void> _showRoster(CampusActivity activity) async {
    await showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) {
        return _RealCreatedRosterSheet(activity: activity);
      },
    );
  }

  int _totalCapacity(List<CampusActivity> activities) {
    return activities.fold(0, (sum, item) => sum + item.capacity);
  }

  int _totalEnrollments(List<CampusActivity> activities) {
    return activities.fold(0, (sum, item) => sum + item.enrolled);
  }

  int _activeCount(List<CampusActivity> activities) {
    return activities.where((item) => !item.isEnded).length;
  }
        ...
        builder: (context, snapshot) {
          ...
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                if (isLoading) const LinearProgressIndicator(minHeight: 2),
                _RealCreatedOverview(
                  total: activities.length,
                  active: _activeCount(activities),
                  enrolled: _totalEnrollments(activities),
                  capacity: _totalCapacity(activities),
                ),
                const SizedBox(height: 14),
                Row(
                  children: [
                ...
                if (isLoading)
                  ...
                  _RealCreatedStateCard(
                    icon: Icons.event_busy_rounded,
                    title: '还没有发起活动',
                    subtitle: '点击右上角加号或下方按钮，创建你的第一场校园活动。',
                    actionLabel: '发起活动',
                  ...
                  for (final activity in activities)
                    Padding(
                      padding: const EdgeInsets.only(bottom: 12),
                      child: _RealCreatedActivityCard(
                        activity: activity,
                        onTap: () => _openDetail(activity),
                        onRoster: () => _showRoster(activity),
                        onResetCode: () => _resetCode(activity),
              ...
              final changed = await Navigator.push<bool>(
                ...
                MaterialPageRoute(
                  builder: (_) => ActivityEnrollmentDetailScreen(
                    ...
                    initialRegistered: item.realRegistered,
                  ),
                ),
              );
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          _ActionButton(
            label: item.realRegistered ? '已报名' : '立即报名',
            color: item.realRegistered ? AppColors.green : AppColors.blue,
            outlined: item.realRegistered,
          ),
        ],
      ),
    );
...
// === Real create/manage activity helpers begin ===

class _RealActivityFormField extends StatelessWidget {
  const _RealActivityFormField({
    required this.controller,
    required this.label,
...
class _RealActivityCategoryDropdown extends StatelessWidget {
  const _RealActivityCategoryDropdown({
    required this.value,
...
class _RealCreatedOverview extends StatelessWidget {
  const _RealCreatedOverview({
    required this.total,
    required this.active,
...
class _RealCreatedOverviewItem extends StatelessWidget {
  const _RealCreatedOverviewItem({
    required this.icon,
    required this.value,
    required this.label,
    required this.color,
...
String _activityStatusLabelFor(CampusActivity activity) {
  if (activity.isEnded) return '已结束';
  if (activity.isCheckedIn) return '已签到';
  if (activity.isCheckInAvailable) return '签到中';
...
Color _activityStatusColorFor(CampusActivity activity) {
  if (activity.isEnded) return AppColors.muted;
  if (activity.isCheckedIn) return AppColors.green;
  if (activity.isCheckInAvailable) return AppColors.blue;
  if (activity.capacity > 0 && activity.enrolled >= activity.capacity) {
  ...
  if (activity.isCheckInNotStarted) return AppColors.purple;
  return AppColors.green;
}

Color _activityStatusBackgroundFor(CampusActivity activity) {
  return _activityStatusColorFor(activity).withValues(alpha: 0.12);
}

bool _activityIsFullFor(CampusActivity activity) {
  return activity.capacity > 0 && activity.enrolled >= activity.capacity;
}

String _activityRegisterActionText(CampusActivity activity, bool registered) {
  if (activity.isEnded) return '活动已结束';
  if (activity.isCheckedIn) return '已签到';
  if (registered) return '已报名';
  if (_activityIsFullFor(activity)) return '名额已满';
  return '立即报名';
}

bool _activityRegisterActionDisabled(CampusActivity activity, bool registered) {
  return activity.isEnded ||
      activity.isCheckedIn ||
      registered ||
      _activityIsFullFor(activity);
}
...
class _RealCreatedActivityCard extends StatelessWidget {
  ...
  double get _progress {
    ...
    return (activity.enrolled / activity.capacity).clamp(0.0, 1.0);
  }
...
class _RealCreatedStatusPill extends StatelessWidget {
  const _RealCreatedStatusPill({required this.label, required this.color});
...
class _RealCreatedActionButton extends StatelessWidget {
  const _RealCreatedActionButton({
    required this.label,
    required this.icon,
    required this.color,
    required this.onTap,
...
class _RealCreatedRosterSheetState extends State<_RealCreatedRosterSheet> {
  ...
  List<CampusActivityEnrollment> _filtered(
    ...
    Iterable<CampusActivityEnrollment> result = items;

    if (_filter == 'checked') {
      result = result.where(_isCheckedIn);
    } else if (_filter == 'unchecked') {
      result = result.where((item) => !_isCheckedIn(item));
    }
              ...
              child: ListView(
                ...
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            _RosterStatusPill(
                              label: _activityStatusLabelFor(activity),
                              color: _activityStatusColorFor(activity),
                            ),
                            const SizedBox(width: 8),
                            Expanded(
                              child: Text(
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        _filterChip('all', '全部', items.length),
                        const SizedBox(width: 8),
                        _filterChip('checked', '已签到', checkedInCount),
                        const SizedBox(width: 8),
                        _filterChip('unchecked', '未签到', notCheckedInCount),
                      ],
                    ),
                  ),
                  const SizedBox(height: 12),
...
class _RealCreatedStateCard extends StatelessWidget {
  const _RealCreatedStateCard({
    required this.icon,
    required this.title,
    required this.subtitle,
...
// === Real create/manage activity helpers end ===
...
class _ActivityEnrollmentDetailScreenState
  ...
  bool get _isFull =>
      _activity.capacity > 0 && _activity.enrolled >= _activity.capacity;
  ...
  Color get _actionButtonColor {
    if (_isSubmitting) return AppColors.muted;
    if (_isCheckedIn) return AppColors.green;
    if (_isEnded) return AppColors.muted;
    if (!_registered && _isFull) return AppColors.orange;
    if (_registered && _isCheckInAvailable) return AppColors.blue;
    if (_registered) return AppColors.red;
    return AppColors.blue;
  }
  ...
  bool get _mainActionEnabled {
    ...
    // 已报名且签到开放：主按钮进入签到页。
    if (_isCheckInAvailable) return true;
  ...
  Future<void> _handleMainAction() async {
    ...
    if (_isCheckInAvailable && !_isCheckedIn && !_isEnded) {
      await _goCheckIn();
      return;
    }
  ...
  Future<void> _cancelRegistration() async {
    if (_isSubmitting || !_registered || _isCheckedIn || _isEnded) return;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text('确定取消报名「${_activity.title}」吗？取消后名额会释放给其他同学。'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false),
    ...
    try {
      final nextActivity = await CampusRepository.instance.cancelJoinActivity(
        _activity,
      );
  ...
  String get _registrationStatusLabel {
    if (_isCheckedIn) return '已签到';
    if (_isEnded) return '已结束';
    if (_registered) return '已报名';
    if (_isFull) return '已满员';
    return '可报名';
  }

  Color get _registrationStatusColor {
    if (_isCheckedIn) return AppColors.green;
    if (_isEnded) return AppColors.muted;
    if (_registered) return AppColors.green;
    if (_isFull) return AppColors.orange;
    return AppColors.orange;
  }

  Future<void> _joinActivity() async {
    if (_isSubmitting || _isFull || _isEnded || _isCheckedIn) return;
    setState(() => _isSubmitting = true);
    try {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Pill(
                  label: _registrationStatusLabel,
                  color: _registrationStatusColor,
                ),
                const Spacer(),
...
class _ActivityCommentSection extends StatefulWidget {
  ...
  @override
  State<_ActivityCommentSection> createState() =>
      _ActivityCommentSectionState();
}
...
class _ActivityCommentSectionState extends State<_ActivityCommentSection> {
  ...
  Future<void> _reload() async {
    final future = CampusRepository.instance.fetchActivityComments(
      widget.activity,
    );
    setState(() => _future = future);
    await future;
  ...
  Future<void> _submit() async {
    ...
    try {
      await CampusRepository.instance.createActivityComment(
        widget.activity,
        text,
      );
      ...
      _controller.clear();
      _showFeatureMessage(context, '评论已发布');

      // 评论发布已经成功，刷新失败不能再提示“操作失败”
      try {
        await _reload();
      } catch (error) {
        debugPrint('评论发布成功，但刷新评论列表失败：$error');
      }
    } catch (error) {
      if (mounted) _showFeatureMessage(context, _featureError(error));
    } finally {
  ...
  Future<void> _delete(CampusComment comment) async {
    ...
    try {
      await CampusRepository.instance.deleteActivityComment(
        widget.activity,
        comment,
      );
      ...
      _showFeatureMessage(context, '评论已删除');

      // 删除已经成功，刷新失败不能再提示“操作失败”
      try {
        await _reload();
      } catch (error) {
        debugPrint('评论删除成功，但刷新评论列表失败：$error');
      }
    } catch (error) {
      if (mounted) _showFeatureMessage(context, _featureError(error));
    }
...
class _ActivityCommentTile extends StatelessWidget {
  const _ActivityCommentTile({required this.comment, required this.onDelete});
                  ...
                  children: [
                    ...
                    PopupMenuButton<String>(
                      ...
                      itemBuilder: (context) => const [
                        PopupMenuItem(value: 'delete', child: Text('删除评论')),
                      ],
                    ),
                  ],
                ),
...
class _ActivityItem {
  ...
  /// 只有后端真实活动才信任报名状态；
  /// 本地 mock / fallback 活动不能显示“已报名”。
  bool get realRegistered => activity != null && registered;
}
